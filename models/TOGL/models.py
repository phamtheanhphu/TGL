...
class TopologyLayer(torch.nn.Module):
    ...
    def compute_persistence(self, x, batch, return_filtration = False):
        ...
        vertex_slices = torch.Tensor(batch['x']).long()
        edge_slices = torch.Tensor(batch['edge_index']).long()
...
class SimpleTopoGNNModel(LargerGCNModel):
    ...
    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        vertex_slices = torch.Tensor(data['x']).cpu().long()
        edge_slices = torch.Tensor(data['edge_index']).cpu().long()
